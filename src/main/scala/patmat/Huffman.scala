...
object Huffman {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def find(tree: CodeTree) (ch:Char): List[Bit] = {
      tree match {
        case Leaf(_, _) => List()
        case Fork(l, r, _, _) if chars(l) contains ch => 1 :: find(l)(ch)
        case Fork(l, r, _, _) => 0 :: find(r)(ch)
      }
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.filter(p => p._1 == char).head._2
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    tree match {
      ...
      case Fork(l, r, lCh, w) => mergeCodeTables(convert(l), convert(r))
    }
  }
